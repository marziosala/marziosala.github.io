...
In this article we look at batch reinforcement learning. The idea is to decouple the data collection and the policy optimization: data is first generated and collected, possibly with a random policy, then this fixed set of experience is used to optimizer or learna policy. Batch algorithms tend to be more data efficient and stable; they also generally ignore the exploration-exploitation problem, doing their best with the data they have. Some problems fit naturally into the batch approach: think of hard-to-obtain data, like medical treatment, emergency response, or the real-life behavior of some complicated robot you don't want to destroy at any new exploration.

A first example is the [Least Square Policy Iteration (LSPI)](http://www.cs.duke.edu/~parr/jmlr03.pdf), which learns a linear approximation of the Q-function. The method is very stable, with not many hyperparameters that need tweaking. It is, however, limited to linear functions over a given set of features. An alternative using deep neural network is called [fitted Q-iterations](https://www.jmlr.org/papers/volume6/ernst05a/ernst05a.pdf). The algorithm is as follows:
1. generate a batch of transitions $\cal{D} = \{(s_i, a_i, r_i, s'_i), i = 1, \ldots, N_{batch}\}$ for some fixed positive number $N_{batch}$;
2. initialize an approximate Q-function $\hat{Q}_\theta$, in our case using deep neural networks, with parameters $\theta$;
3. initialize the training set $\cal{T}$;
4. for each $(s_i, a_i, r_i, s'_i) \in \cal{D}$:
    - $\hat{q}_i = 
        \begin{cases}
            r_i + \gamma \max_{a'} \hat{Q}_\theta(s'_i, a_i) & \text{if}\ s_i \text{ is not terminal} \\
            r_i & \text{otherwise}
        \end{cases}$,
    - add the training sample$\{ (s_i, a_i), \hat{q}_i \}$ to $\cal{T}$;
5. learn $\hat{Q}_\theta$ from $\cal{T}$;
6. goto step 3.
...
![png](/assets/images/acrobot/acrobot_8_1.png)
...
![png](/assets/images/acrobot/acrobot_18_1.png)


To finish the article, we generate a small video.
