...
In this article we look at [embeddings](https://en.wikipedia.org/wiki/Word_embedding) with the aim of estimating the similarity between different texts. An embedding is a vector representation of a word or a text. The idea is to transform a set of words $T$ into a mathematical object $x \in \mathbb{R}^n$, with $n$ large, say a thousand or more. As concrete use case, we look at the articles published on [arxiv.org](https://www.arxiv.org); the embeddings are provided by [OpenAI](https://www.openai.com).
