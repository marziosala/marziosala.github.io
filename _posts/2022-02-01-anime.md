...
In a GAN, the generator takes in input a vector $\xi$ of noise and produces fake examples $\hat{X}$. The fakes $\hat{X}$ as well as some real images $X$ are passed to the discriminator, which is trained to distinguish fake and real images. This is done using $X$, $\hat{X}$ and the corresponding labels using the binary cross entropy (BCE) loss to update the parameters of the discriminator.

BCE is designed for classification tasks where there are two outcomes (in our case, real and fake). The loss function is
$$
...
The formula looks complicated but is quite simple. Let's take a look at the
$y^{(i)} \log h(x^{(i)}; \vartheta)$ term first, which is the product of the true
...
Torch transformers are very easy to set up: we resize all the images to be the size we want, we center them, transform to tensor and make sure they are normalized in the $[-1, 1]$ range. Using CUDA is essential for a (relatively) quick training.  
...
This is the generator class. Transpose convolutions are used to increase the image resolution. The last layer is the tanh unit as the outputs should be in the $[-1, 1]$ range.
...
And this is the discriminator class. Last layer is the sigmoid unit as outputs should be in the $[0, 1]$ range.
...
As discussed above, we use the BCE loss function.
