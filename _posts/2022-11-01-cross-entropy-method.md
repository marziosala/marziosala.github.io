...
Finally, we check for convergence: if $\max{|\sigma_{t, j}|} < \epsilon_{tol}$, stop and return $\mu_t$ or the overall best solution generated by the algorithm.
