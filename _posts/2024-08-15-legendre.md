...
- the *Abel* kernel $K(x, y) = e^{-\alpha | x - y |}, \alpha > 0, d = 1$.
...
Let's focus first on the first step. We will indicate with $$\{ \psi_b(x, y) : \Omega \rightarrow \mathbb{R}, b=1\, \ldots, B \}$$ the set of basis functions of such space and operate exclusively in this space.
...
To apply the method just described we use a two-dimensional Gaussian process.
...
samples_hat = projection.inverse_transform(A)
```
...
axes[1, 0].plot_surface(X, Y, samples_hat[10], cmap='viridis')
...
axes[1, 1].plot_surface(X, Y, samples_hat[20], cmap='viridis')
...
axes[1, 2].plot_surface(X, Y, samples_hat[30], cmap='viridis')
...
As expected, as $L$ increases the quality of the fit improves.
...
axes[0, 1].plot_surface(X, Y, fda.inverse_transform(fda.transform(samples_hat[10]), 1), cmap='viridis')

axes[0, 2].set_title('Reconstructed Sample, L=2')
axes[0, 2].plot_surface(X, Y, fda.inverse_transform(fda.transform(samples_hat[10]), 2), cmap='viridis')
axes[1, 0].set_title('Reconstructed Sample, L=4')
axes[1, 0].plot_surface(X, Y, fda.inverse_transform(fda.transform(samples_hat[10]), 4), cmap='viridis')

axes[1, 1].set_title('Reconstructed Sample, L=6')
axes[1, 1].plot_surface(X, Y, fda.inverse_transform(fda.transform(samples_hat[10]), 6), cmap='viridis')
axes[1, 2].set_title('Reconstructed Sample, L=15')
axes[1, 2].plot_surface(X, Y, fda.inverse_transform(fda.transform(samples_hat[10]), 15), cmap='viridis');
```
