...
we have
$$
V_t = \{V_{t, i} = z(t, x_i, y_i), i=1, \ldots, I\}.
$$
For simplicity we assume $(x, y) \in \Omega = [-1, 1] \times [-1, 1]$, assuming that different domains can be mapped to $\Omega$.

Given the finite sequence $$\{C_t\}$$, we can produce an estimate of the covariance $\hat C(x, y, u, v)$ of the process between two sets of points $(x, y), (u, v) \in \Omega$ and then apply Mercer's theorem to it to identify the most important modes of such covariance operator. This means that we will identify a set of functions $$\{\varphi_\ell(x, y) : \Omega \rightarrow \mathbb{R}, \ell=1,\ldots, L\}$$ that explain most of the variance in the process. To do this numerically, the first step is to approximate them into a finite-dimensional space, since both $K$ and $\varphi_\ell$, being continuous, are infinite-dimensional. We will indicate with $$\{ \psi_b(x, y) : \Omega \rightarrow \mathbb{R}, b=1\, \ldots, B \}$$ the set of basis functions of such space and operate exclusively in this space.
