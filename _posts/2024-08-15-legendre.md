...
\{ X_t, t=1, \ldots, T\},
$$
where each $X_t$ is a sample of an unknown function $X(t, x, y)$, that is given a set of sampling points $\{x_i, y_i, i=1, \ldots, I \},$ we have $X_t = \left\{X_{t, i} = z(t, x_i, y_i), i=1, \ldots, I\right\}$. For simplicity we assume $(x, y) \in \Omega = [-1, 1] \times [-1, 1]$, knowing that different domains can be easily mapped to $\Omega$.
